...
exports.register = async (req, res, next) => {
  try {
    ...
    const exists = await User.findOne({ $or: [{ email: email.toLowerCase() }, { username: lowerUsername }] });
    if (exists) return next(createError(409, 'User already exists'));
    const user = await User.create({ username: lowerUsername, name, email: email.toLowerCase(), password, role: role || 'security' });
