...
exports.checkIn = async (req, res, next) => {
  try {
    ...
    // Emit WebSocket event for visitor check-in
    if (global.io) {
...
exports.checkOut = async (req, res, next) => {
  try {
    ...
    // Emit WebSocket event for visitor check-out
    if (global.io) {
...
exports.updateVisitorStatus = async (req, res, next) => {
  try {
    ...
    // Additional side effects (e.g., clearing OTPs if rejected)
    if (status === 'rejected' || status === 'denied') {
      const OTP = require('../models/OTP');
      await OTP.updateMany({ appointmentId: visitor._id }, { $set: { used: true } });
    }
