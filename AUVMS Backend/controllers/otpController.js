...
exports.generate = async (req, res, next) => {
  try {
    ...
    if (visitor.email) {
      sendEmail({ to: visitor.email, subject: 'Your Aditya University Appointment OTP', html: `<p>Hello ${visitor.name},</p><p>Your appointment with <b>${visitor.personToMeet}</b> on <b>${dateStr}</b> at <b>${timeStr}</b> has been approved.</p><p>Your OTP for campus entry is: <b>${otp}</b></p>` }).catch(() => { });
    }
...
exports.verify = async (req, res, next) => {
  try {
    ...
    // Remove expiry check
    const candidates = await OTP.find({ used: false }).sort({ createdAt: -1 }).limit(500);
    let matched = null;
